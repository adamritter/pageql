...
<script>
  ...
  function maybe_load_more(el,mid){var can=true;function h(){if(!can)return;var sp=(el===window||el===document.body?window.scrollY+window.innerHeight:el.scrollTop+el.clientHeight);var sh=(el===window||el===document.body?document.documentElement.scrollHeight:el.scrollHeight)-150;if(sp>=sh){var msg='infinite_load_more '+mid;if(window.pageqlSocket&&window.pageqlSocket.readyState===1)window.pageqlSocket.send(msg);else{if(!window.pageqlSendQueue)window.pageqlSendQueue=[];window.pageqlSendQueue.push(msg);}can=false;}};(el===window||el===document.body?window:el).addEventListener('scroll',h);}
