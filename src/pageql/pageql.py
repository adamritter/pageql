    ...
    def ensure_init(self, out):
        if not self.initialized:
            out.append(
                ...
                "function pupdate(o,n,v){var m=window.pageqlMarkers[o],e=m.e,r=document.createRange();r.setStartAfter(m);r.setEndBefore(e);r.deleteContents();var t=document.createElement('template');t.innerHTML=v;e.parentNode.insertBefore(t.content,e);m.data='pageql-start:'+n;m.e.data='pageql-end:'+n;delete window.pageqlMarkers[o];window.pageqlMarkers[n]=m;}"
            ...
            if directive == '#if':
                ...
                if ctx and reactive:
                    def on_event(ev, *, mid=mid, out=output_buffer, ctx=ctx,
                                 col_names=col_names, body=body, path=path,
                                 includes=includes, http_verb=http_verb,
                                 reactive=reactive, saved_params=saved_params):
                        if ev[0] == 2:
                            ...
                            ctx.ensure_init(out)
                            out.append(
                                f"<script>pupdate('{old_id}','{new_id}',{json.dumps(row_content)})</script>"
                            )
